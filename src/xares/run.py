...
def worker(
    ...
    if encoder is not None:
        config.encoder_name = get_encoder_run_name(
            encoder=encoder,
            encoder_py_path=encoder_py,
        )
    if config.disabled:
...
def main(args):
    ...
    ckpt_name = get_encoder_run_name(
        encoder=encoder,
        encoder_py_path=args.encoder_py,
    ...
    if args.from_stage <= 2 and args.to_stage >= 2:
        ...
        df[["Task", "MLP_Score", "Private"]].to_csv(
            mlp_dir / f"{ckpt_name}_MLP.csv",
            index=False,
        )
        df[["Task", "KNN_Score", "Private"]].to_csv(
            knn_dir / f"{ckpt_name}_KNN.csv",
            index=False,
        )
